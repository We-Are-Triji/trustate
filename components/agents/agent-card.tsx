      ...
      <CardContent className="p-0 relative">
        ...
        <div className="relative bg-gradient-to-br from-[#0247ae] via-[#0560d4] to-[#0873c9] h-36 overflow-hidden">
          ...
          <div className="absolute left-4 top-4 z-10">
            <div className="flex items-center gap-2 rounded-xl bg-white/95 backdrop-blur-md px-3.5 py-2 shadow-lg border border-white/50">
              <span className={`h-2 w-2 rounded-full ${getStatusColor(agent.status)} ${agent.status === 'active' ? 'animate-pulse' : ''}`} />
              <span className="text-xs font-extrabold text-gray-800">{getStatusLabel(agent.status)}</span>
            </div>
          </div>
          ...
          <div className="absolute right-4 top-4 z-10">
            <div className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#ffce08] to-[#f5c000] px-3.5 py-2 shadow-lg border border-yellow-300/50">
              ...
              <span className="text-xs font-extrabold text-[#0247ae]">{agent.rating}</span>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 pb-6 pt-8">
          ...
          <div className="text-center mb-5">
            <h3 className="text-xl font-extrabold text-gray-900 mb-1.5 group-hover:text-[#0247ae] transition-colors duration-300 leading-tight">{agent.name}</h3>
            <p className="text-sm font-bold bg-gradient-to-r from-[#0247ae] to-[#0560d4] bg-clip-text text-transparent mb-1.5">{agent.title}</p>
            <p className="text-xs text-gray-500 font-semibold">{agent.employeeCode}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            <div className="text-center p-4 rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-100 transition-all duration-300 hover:border-blue-200 hover:shadow-md">
              <div className="flex items-center justify-center gap-1.5 mb-1.5">
                ...
                <p className="text-xs font-bold text-gray-600">Reviews</p>
              </div>
              <p className="text-xl font-extrabold text-[#0247ae]">{agent.reviewCount}</p>
            </div>
            <div className="text-center p-4 rounded-2xl bg-gradient-to-br from-yellow-50 to-orange-50 border-2 border-yellow-100 transition-all duration-300 hover:border-yellow-200 hover:shadow-md">
              <div className="flex items-center justify-center gap-1.5 mb-1.5">
                ...
                <p className="text-xs font-bold text-gray-600">Rating</p>
              </div>
              <p className="text-xl font-extrabold text-[#ffce08]">{agent.rating}</p>
            </div>
          </div>

          {/* Tags */}
          <div className="flex flex-wrap items-center justify-center gap-2 mb-5">
            <span className="rounded-xl bg-gradient-to-r from-[#0247ae]/10 to-[#0560d4]/10 px-4 py-2 text-xs font-extrabold text-[#0247ae] border-2 border-[#0247ae]/20 whitespace-nowrap">
              {getCategoryLabel(agent.category)}
            </span>
            <span className="rounded-xl bg-gray-100 px-4 py-2 text-xs font-extrabold text-gray-700 border-2 border-gray-200 whitespace-nowrap">
