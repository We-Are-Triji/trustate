...
export function ActivityTab({ transactionId }: ActivityTabProps) {
    ...
    const fetchLogs = async () => {
        try {
            const response = await fetch(`/api/transactions/${transactionId}/activity`);
            if (response.ok) {
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="p-6 border-b border-gray-100 bg-gray-50/50">
                ...
                <p className="text-sm text-gray-500">Comprehensive timeline of all transaction events</p>
            </div>
                            ...
                            <div key={log.id} className="relative z-10 flex gap-4">
                                ...
                                <div className="flex-1 pt-1">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {/* Optional metadata display could go here */}
                                    </div>
                                </div>
                            </div>
                        ))
