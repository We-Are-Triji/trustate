      ...
      <CardContent className="p-0 relative">
        ...
        <div className="relative bg-gradient-to-br from-[#0247ae] via-[#0560d4] to-[#0873c9] h-24 md:h-36 overflow-hidden">
          ...
          <div className="absolute left-2 md:left-4 top-2 md:top-4 z-10">
            <div className="flex items-center gap-1.5 md:gap-2 rounded-lg md:rounded-xl bg-white/95 backdrop-blur-md px-2 md:px-3.5 py-1 md:py-2 shadow-lg border border-white/50">
              <span className={`h-1.5 md:h-2 w-1.5 md:w-2 rounded-full ${getStatusColor(broker.status)} ${broker.status === 'active' ? 'animate-pulse' : ''}`} />
              <span className="text-[10px] md:text-xs font-extrabold text-gray-800">{getStatusLabel(broker.status)}</span>
            </div>
          </div>
          ...
          <div className="absolute right-2 md:right-4 top-2 md:top-4 z-10">
            <div className="flex items-center gap-1.5 md:gap-2 rounded-lg md:rounded-xl bg-gradient-to-r from-[#ffce08] to-[#f5c000] px-2 md:px-3.5 py-1 md:py-2 shadow-lg border border-yellow-300/50">
              ...
              <span className="text-[10px] md:text-xs font-extrabold text-[#0247ae]">{broker.rating}</span>
            </div>
          </div>
        </div>
        ...
        <div className="px-3 md:px-6 pb-3 md:pb-6 pt-4 md:pt-8">
          ...
          <div className="text-center mb-3 md:mb-5">
            <h3 className="text-base md:text-xl font-extrabold text-gray-900 mb-1 md:mb-1.5 group-hover:text-[#0247ae] transition-colors duration-300 leading-tight">{broker.name}</h3>
            <p className="text-xs md:text-sm font-bold bg-gradient-to-r from-[#0247ae] to-[#0560d4] bg-clip-text text-transparent mb-1 md:mb-1.5">{broker.title}</p>
            <p className="text-[10px] md:text-xs text-gray-500 font-semibold">{broker.licenseNumber}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 md:gap-3 mb-3 md:mb-5">
            <div className="text-center p-2 md:p-4 rounded-xl md:rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-100 transition-all duration-300 hover:border-blue-200 hover:shadow-md">
              <div className="flex items-center justify-center gap-1 md:gap-1.5 mb-1 md:mb-1.5">
                ...
                <p className="text-[10px] md:text-xs font-bold text-gray-600">Sold</p>
              </div>
              <p className="text-base md:text-xl font-extrabold text-[#0247ae]">{broker.propertiesSold}</p>
            </div>
            <div className="text-center p-2 md:p-4 rounded-xl md:rounded-2xl bg-gradient-to-br from-yellow-50 to-orange-50 border-2 border-yellow-100 transition-all duration-300 hover:border-yellow-200 hover:shadow-md">
              <div className="flex items-center justify-center gap-1 md:gap-1.5 mb-1 md:mb-1.5">
                ...
                <p className="text-[10px] md:text-xs font-bold text-gray-600">Experience</p>
              </div>
              <p className="text-base md:text-xl font-extrabold text-[#ffce08]">{broker.yearsExperience}+</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-1.5 md:gap-2 mb-3 md:mb-5">
            {broker.specializations.slice(0, 2).map((spec) => (
              <span
                key={spec}
                className="rounded-lg md:rounded-xl bg-gradient-to-r from-[#0247ae]/10 to-[#0560d4]/10 px-2 md:px-4 py-1 md:py-2 text-[10px] md:text-xs font-extrabold text-[#0247ae] border-2 border-[#0247ae]/20 whitespace-nowrap"
              >
                {spec}
              </span>
            ))}
