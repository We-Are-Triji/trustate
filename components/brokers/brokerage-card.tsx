...
export function BrokerageCard({ brokerage, onClick }: BrokerageCardProps) {
  ...
  return (
    <Card
      className="group cursor-pointer overflow-hidden rounded-2xl md:rounded-3xl border-0 bg-white shadow-lg transition-all duration-500 hover:shadow-[0_20px_60px_-15px_rgba(2,71,174,0.35)] hover:-translate-y-3 p-0 gap-0"
    ...
    >
      <CardContent className="p-0 relative">
        ...
        <div className="relative h-32 md:h-48 overflow-hidden">
          ...
          <div className="absolute left-2 md:left-4 top-2 md:top-4 z-10">
            <div className="flex items-center gap-1.5 md:gap-2 rounded-lg md:rounded-xl bg-white/95 backdrop-blur-md px-2 md:px-3.5 py-1 md:py-2 shadow-lg border border-white/50">
              <span className={`h-1.5 md:h-2 w-1.5 md:w-2 rounded-full ${getStatusColor(brokerage.status)} ${brokerage.status === 'active' ? 'animate-pulse' : ''}`} />
              <span className="text-[10px] md:text-xs font-extrabold text-gray-800">{getStatusLabel(brokerage.status)}</span>
            </div>
          </div>
          ...
          <div className="absolute right-2 md:right-4 top-2 md:top-4 z-10">
            <div className="flex items-center gap-1.5 md:gap-2 rounded-lg md:rounded-xl bg-gradient-to-r from-[#ffce08] to-[#f5c000] px-2 md:px-3.5 py-1 md:py-2 shadow-lg border border-yellow-300/50">
              ...
              <span className="text-[10px] md:text-xs font-extrabold text-[#0247ae]">{brokerage.rating}</span>
              <span className="text-[10px] md:text-xs font-bold text-[#0247ae]">({brokerage.reviewCount})</span>
            </div>
          </div>
          
          {/* Brokerage name overlay at bottom */}
          <div className="absolute bottom-0 left-0 right-0 p-2 md:p-4 z-10">
            <h3 className="text-base md:text-xl font-extrabold text-white mb-0.5 md:mb-1 leading-tight drop-shadow-lg">{brokerage.name}</h3>
        ...
        {/* Brokerage info */}
        <div className="px-3 md:px-6 pb-3 md:pb-6 pt-3 md:pt-6">
          {/* Description */}
          <p className="text-xs md:text-sm text-gray-600 mb-3 md:mb-5 line-clamp-2 leading-relaxed">
          ...
          <div className="grid grid-cols-3 gap-2 md:gap-3 mb-3 md:mb-5">
            <div className="text-center p-2 md:p-4 rounded-xl md:rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-100 transition-all duration-300 hover:border-blue-200 hover:shadow-md">
              <div className="flex items-center justify-center gap-0.5 md:gap-1 mb-0.5 md:mb-1">
                <Users className="h-3 md:h-4 w-3 md:w-4 text-[#0247ae]" />
              </div>
              <p className="text-lg md:text-2xl font-extrabold text-[#0247ae] mb-0.5">{brokerage.agentCount}</p>
              <p className="text-[8px] md:text-[10px] font-bold text-gray-600 uppercase tracking-wider">Agents</p>
            </div>
            <div className="text-center p-2 md:p-4 rounded-xl md:rounded-2xl bg-gradient-to-br from-yellow-50 to-orange-50 border-2 border-yellow-100 transition-all duration-300 hover:border-yellow-200 hover:shadow-md">
              <div className="flex items-center justify-center gap-0.5 md:gap-1 mb-0.5 md:mb-1">
                <Home className="h-3 md:h-4 w-3 md:w-4 text-[#ffce08]" />
              </div>
              <p className="text-lg md:text-2xl font-extrabold text-[#ffce08] mb-0.5">{brokerage.propertiesListed}</p>
              <p className="text-[8px] md:text-[10px] font-bold text-gray-600 uppercase tracking-wider">Listings</p>
            </div>
            <div className="text-center p-2 md:p-4 rounded-xl md:rounded-2xl bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-100 transition-all duration-300 hover:border-green-200 hover:shadow-md">
              <div className="flex items-center justify-center gap-0.5 md:gap-1 mb-0.5 md:mb-1">
                <Award className="h-3 md:h-4 w-3 md:w-4 text-emerald-600" />
              </div>
              <p className="text-lg md:text-2xl font-extrabold text-emerald-600 mb-0.5">{brokerage.yearsInBusiness}+</p>
              <p className="text-[8px] md:text-[10px] font-bold text-gray-600 uppercase tracking-wider">Years</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-1.5 md:gap-2 mb-3 md:mb-5">
            {brokerage.specializations.slice(0, 3).map((spec) => (
              <span
                key={spec}
                className="rounded-lg md:rounded-xl bg-gradient-to-r from-[#0247ae]/10 to-[#0560d4]/10 px-2 md:px-3 py-1 md:py-1.5 text-[10px] md:text-xs font-extrabold text-[#0247ae] border-2 border-[#0247ae]/20 whitespace-nowrap"
              >
                {spec}
              </span>
            ))}
          ...
          {/* Account Manager */}
          <div className="mb-3 md:mb-6 p-2.5 md:p-5 rounded-xl md:rounded-2xl bg-gradient-to-br from-gray-50 to-white border-2 border-gray-100">
            <p className="mb-2 md:mb-3 text-[10px] md:text-xs font-extrabold uppercase tracking-wider text-gray-500 text-center">
              Account Manager
            </p>
            <div className="flex items-center gap-2 md:gap-3">
              <div className="flex-shrink-0 h-10 w-10 md:h-14 md:w-14 rounded-full border-[2px] md:border-[3px] border-[#0247ae]/20 overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 shadow-md transition-all duration-300 group-hover:border-[#ffce08] group-hover:scale-105">
                <img 
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs md:text-sm font-extrabold text-gray-800 truncate">
                  {brokerage.representative.name}
                </p>
                <p className="text-[10px] md:text-xs text-gray-500 truncate font-semibold">
