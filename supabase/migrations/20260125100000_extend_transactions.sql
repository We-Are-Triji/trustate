...
-- Add transaction_type enum (safely)
DO $$ BEGIN
...
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS transaction_type transaction_type DEFAULT 'preselling';
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS unit_address TEXT;
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS client_name TEXT;
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS reservation_number TEXT;
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS lifecycle_step INTEGER DEFAULT 1 CHECK (lifecycle_step >= 1 AND lifecycle_step <= 6);
...
CREATE INDEX IF NOT EXISTS idx_transactions_lifecycle_step ON transactions(lifecycle_step);
